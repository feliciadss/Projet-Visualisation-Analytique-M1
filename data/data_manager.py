...
class DataManager:
    ...
    def create_genre_popularity_by_country(self, selected_genre):
        ...
        df = pd.DataFrame(
            list(country_popularity.items()), columns=["country", "total_popularity"]
        )
    ...
    def create_genre_collaboration_matrix(self, selected_genres):
        ...
        genre_matrix = pd.crosstab(
            df_collaborations["Genre1"], df_collaborations["Genre2"]
        )
        return genre_matrix
    ...
    def create_genre_count_dataframe(self):
        ...
        df_genre_count = pd.DataFrame(
            list(genre_count.items()), columns=["genre", "total_count"]
        )
    ...
    def get_festival_data(self):
        try:
            ...
            df_festivals["Participants (approx)"] = (
                df_festivals["Participants (approx)"]
                .replace(",", "", regex=True)
                .astype(int)
            )
            ...
            df_festivals["Prix moyen (€)"] = (
                df_festivals["Prix moyen (€)"].replace(",", "", regex=True).astype(int)
            )

            # Optionnel : nettoyage des colonnes de texte
            df_festivals["Genres musicaux"] = df_festivals[
                "Genres musicaux"
            ].str.strip()
            df_festivals["Pays"] = df_festivals["Pays"].str.strip()
            df_festivals["Nom du festival"] = df_festivals[
                "Nom du festival"
            ].str.strip()
    ...
    def get_top_collabs_between_genres(self, genre1, genre2, top_n=10):
        ...
        if not collabs_data:
            print(
                f"Aucune collaboration trouvée entre les genres {genre1} et {genre2}."
            )
            return pd.DataFrame()
        ...
        df_collabs = df_collabs.drop(
            columns=["collab_popularity", "track_id"], errors="ignore"
        )
