...
class UpdateSQLManager:
    ...
    def update_artists(self):
        ...
        for artist in artists:
            ...
            followers_total = (
                followers.get("total", 0) if isinstance(followers, dict) else followers
            )

            cursor.execute(
                """INSERT OR REPLACE INTO artists (id, name, popularity, followers, genres, market)
                            VALUES (?, ?, ?, ?, ?, ?)""",
                (
                    artist["id"],
                    artist.get("name", ""),
    ...
    def update_tracks(self):
        ...
        for track in tracks:
            ...
            cursor.execute(
                """INSERT OR REPLACE INTO tracks (id, name, album_id, tempo, energy, danceability, acousticness, 
    ...
    def update_albums(self):
        ...
        for album in albums:
            ...
            cursor.execute(
                """INSERT OR REPLACE INTO albums (id, name, release_date, available_markets, total_tracks, artist_id) 
                              VALUES (?, ?, ?, ?, ?, ?)""",
                (
                    album["id"],
                    album.get("name", ""),
