...
articles = [
    html.Div(
        children=[
            html.Img(
                src=f"/static/images/{genre}.jpg",
                style={"width": "100%", "border-radius": "5px"},
            ),
            html.A(
                f"L'histoire {article} {genre.capitalize()}",
                href=link,
                target="_blank",
                style={
                    "display": "block",
        ...
        html.Div(
            ...
            children=[
                ...
                dcc.Store(
                    ...
                    data={genre: genre in ["rock", "r&b"] for genre in genres},
                ),
            ],
        ),
...
@callback(Output("linear-graph", "figure"), Input("selected-genres-collab", "data"))
def update_content_evolution(selected_genres):
    ...
    if "release_date" in df.columns:
        ...
        albums_per_year = (
            df.groupby(["year", "genre"]).size().reset_index(name="album_count")
        )
    else:
